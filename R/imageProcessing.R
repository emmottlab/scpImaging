...
# 2. npyMasksToPng() - Status: functional (some maxColorValue errors (3% of files)) -
...
#'
#' @param df A data frame containing cellenONE-derived image metadata. For example a cell sorting file
#'  generated by the cellenONE, or a sampleannotation file generated by scpAnnotator. **Required.**
#'   Must include columns: `ImageFile` (character, full path to the input image
#'   or filename relative to `input_dir` if provided),
#'   `X` (numeric, x-coordinate of the center point of the cell for cropping, as identified by the cellenONE),
#'   `Y` (numeric, y-coordinate of the center point of the cell for cropping, as identified by the cellenONE).
...
#' @param output_prefix A character string to add to the beginning of the output filenames.
#'   Defaults to `"cropped_"'. Optional. **Strongly recommend** leaving unchanged for consistency.
...
#'    **Strongly recommend** leaving unchanged for consistency.
...
#'
#' @return Returns `NULL`, so does not create an object in the R environment. The function's primary effect is saving
...
##############################################################################################################
#' 'npyMasksToPng()' converts Cellpose.npy Masks to PNG Images
#'
...
#'   with the 'numpy' library installed to access the .npy files. It reads the
#'   .npy files and converts to .png images, where the background is black,
#'   and each cell mask has a unique colour.
#'
#' @export
...
#'   results <- npyMasksToPng(input_dir = temp_in_dir, output_dir = temp_out_dir)
#'
...
#' \dontshow{\}) # examplesIf}
npyMasksToPng <- function(input_dir,
                             output_dir = NULL,
...
#' 'overlayMask()': Overlay Segmentation Mask on Image (Single File or Directory)
#'
...
#' overlay on the original image. Uses the 'magick' package.
#'
...
#' Default is `"_cp_masks_overlay.png"`. **Strongly recommend** not changing.
...
overlayMask <- function(image_input,
  ...
  if (!is_image_dir && !is_mask_dir) {
    ...
    if (is.null(image_suffix)) {
      ...
      ImageFiles_list <- list.files(image_dir, pattern = image_pattern, full.names = TRUE)
    }

    if (length(ImageFiles_list) == 0) warning("No image files found matching criteria in: ", image_dir, call. = FALSE)
    ...
    image_bases <- NULL
    if (is.null(image_suffix)) {
      image_bases <- tools::file_path_sans_ext(basename(ImageFiles_list))
    } else {
      image_bases <- sapply(ImageFiles_list, get_base_name, suffix = image_suffix)
    }
    names(image_bases) <- ImageFiles_list # Keep track of full paths
    ...
    if (length(unmatched_image_bases) > 0) {
      unmatched_ImageFiles <- names(image_bases)[image_bases %in% unmatched_image_bases]
      warning("Found ", length(unmatched_ImageFiles), " image file(s) with no matching mask file:\n ", paste(basename(unmatched_ImageFiles), collapse="\n "), call. = FALSE)
    }
    ...
    if (length(common_bases) == 0 && (length(ImageFiles_list) > 0 || length(mask_files_list) > 0)) {
...
##############################################################################################################
#' 'overlayMaskOnParent()': Overlay Segmentation Masks onto Parent Images at Specific Coordinates
#'
...
#' @param mode Character string: "outline" or "overlay". Determines the
#' visualization style. Default is "outline".
#' @param parent_image_suffix Character string. Suffix expected for parent image
#' files (e.g., "_Run.png"). Used to find parent images and derive base names.
#' Default is "_Run.png".
#' @param mask_suffix Character string. The final suffix part of the mask filename
#' (e.g., "_cp_masks.png"). Default is "_cp_masks.png".
#' @param output_suffix Character string. Suffix appended to the determined parent
#' base name for the generated output files. Default is "_parent_overlayed.png". **Strongly recommend** not changing.
...
overlayMaskOnParent <- function(parent_image_input,
 ...
 # Validate input types and existence
  ...
  if (!is.character(coord_df$ImageFile)) {
    stop("Column 'ImageFile' in coord_df must be character strings.")
  }
 ...
 # --- Group coordinates by Parent File (using ImageFile column) ---
    if (any(duplicated(coord_df$ImageFile))) {
      warning("Duplicate entries found in 'ImageFile' column. Processing all entries.", call. = FALSE)
    }

  parent_groups <- split(coord_df, coord_df$ImageFile)
  unique_parent_identifiers <- names(parent_groups)

  output_paths <- list()
  processed_parent_count <- 0
  total_masks_processed_successfully <- 0

  message("Found ", length(unique_parent_identifiers), " unique parent image identifier(s) in coord_df$ImageFile.")

 # --- Loop through each Parent Image Identifier ---
    for (parent_identifier_from_df in unique_parent_identifiers) {
      parent_coords <- parent_groups[[parent_identifier_from_df]]

      # --- Determine Parent File Path and Base Name ---
      parent_file_path <- NULL
      parent_base_name_for_output <- NULL

      intended_file_name <- paste0(parent_identifier_from_df, parent_image_suffix)
      intended_file_path <- file.path(parent_image_input, intended_file_name)

      alternative_file_name <- parent_identifier_from_df
      alternative_file_path <- file.path(parent_image_input, alternative_file_name)

      if (file.exists(intended_file_path)) {
        parent_file_path <- intended_file_path
        parent_base_name_for_output <- parent_identifier_from_df
      } else if (file.exists(alternative_file_path) && endsWith(alternative_file_name, parent_image_suffix)) {
        parent_file_path <- alternative_file_path
        parent_base_name_for_output <- sub(paste0(parent_image_suffix, "$"), "", alternative_file_name)
      } else {
        warning("Parent image file not found for identifier '", parent_identifier_from_df,
                "'. Looked for: '", intended_file_path, "' and potentially '", alternative_file_path,
                "'. Skipping this identifier.", call. = FALSE)
        next
      }
      # --- End Parent Path Logic ---

      current_parent_state <- NULL
      parent_processed_successfully = TRUE
      ...
      tryCatch({
        ...
        for (i in 1:nrow(parent_coords)) {
          ...
          if (is.null(parent_base_name_for_output)) {
            warning("Could not determine base name for identifier '", parent_identifier_from_df, "'. Cannot construct mask path.", call. = FALSE)
            next # Skip if base name derivation failed
          }
          mask_file_name <- paste0("cropped_", parent_base_name_for_output, "_Run", mask_suffix)
          mask_file_path <- file.path(mask_input, mask_file_name)
          # --- End CORRECTED Mask Path ---


          if (!file.exists(mask_file_path)) {
            # Use the derived base name in the warning for clarity
            warning("Mask file not found for parent base '", parent_base_name_for_output,
                    "', skipping overlay at X=", coord_info$X, ", Y=", coord_info$Y,
                    ". Looked for: ", mask_file_path, call. = FALSE)
            next
          }
          ...
          tryCatch({
            ...
            if (mode == "outline") {
              ...
              mask_transparent <- magick::image_transparent(mask_img, color = "black", fuzz = 10)
              rgb_vals <- grDevices::col2rgb(overlay_col)
              # Corrected rgba string formatting
              semi_transparent_color_string <- sprintf("rgba(%d, %d, %d, %f)",
                                                       rgb_vals["red", 1],   # Extract Red value
                                                       rgb_vals["green", 1], # Extract Green value
                                                       rgb_vals["blue", 1],  # Extract Blue value
                                                       overlay_alpha)
        ...
        } # End inner loop through masks/coordinates

        # --- Save the final parent image ---
        if (masks_processed_for_this_parent > 0) {
          output_base_name <- paste0(parent_base_name_for_output, output_suffix)
        ...
        warning(paste("Failed to read/process parent image '", basename(parent_file_path), "'. Skipping this parent.\n Error: ", e_parent$message), call. = FALSE)
        parent_processed_successfully = FALSE
    ...
    } # End outer loop through parent identifiers
